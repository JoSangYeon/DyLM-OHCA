...
class GPT_Baseline(GPT2Model):
    def __init__(self, config, class_num=2, pad_token_id=0,
                 cls_token_id=2, sep_token_id=3, is_avgpool=False):
        ...
        self.ctc_classifier = nn.Sequential(OrderedDict([
            ('dense',nn.Linear(self.embed_dim, self.embed_dim)),
            ('dropout', nn.Dropout(0.1)),
            ('out_proj', nn.Linear(self.embed_dim, class_num+1)),
        ]))
    ...
    def get_ctc_loss(self, ctc_logits, ctc_logits_length, target, target_length):
        return self.criterion(ctc_logits, target, ctc_logits_length, target_length)
    ...
    def forward(
        ...
        target_length: Optional[torch.LongTensor] = None,
        ...
        logits = self.ctc_classifier(hidden_states) # batch, len, dim -> batch, len, class_num
        
        ctc_logits = logits.permute(1, 0, 2)   # For CTCLoss (len, batch, class_num)
        ctc_logits = F.log_softmax(ctc_logits, 2)  # For CTCLoss (len, batch, class_num)
        ctc_logits_length = torch.full(size=(ctc_logits.size(1),), fill_value=ctc_logits.size(0)).long().to(device) # [batch_size]
        ctc_loss = self.get_ctc_loss(ctc_logits, ctc_logits_length, target, target_length)
        ...
        return logits[:, :, 1:], logit[:, 1:], ctc_loss
