...
def main(rank, world_size, port, seed, args):
    ...
    # Define project
    project_name = f'NIA_119-GPT_CTC_Loss'
    model_name = 'only_Text_GPT_CTC_{}_{}'.format("full" if is_full else "end", 'avgpool' if is_avgpool else 'last')
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, 
                              sampler=train_sampler, num_workers=world_size*4)
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, 
                              sampler=valid_sampler, num_workers=world_size*4)
    ...
    # modeling
    model = GPT_Baseline.from_pretrained(model_link, class_num=class_num, is_avgpool=is_avgpool,
    ...
    if rank==0:
        ...
        for model_path in ckpt_path:
            ...
            model = GPT_Baseline.from_pretrained(model_link, class_num=class_num, is_avgpool=is_avgpool, pad_token_id=pad_token_id, 
            ...
            ((label_1_predicted_probas, label_0_predicted_probas, labels), 
             (file_ids_list, input_ids_list, spk_type_ids_list, 
              label_1_predicted_token_logits, label_0_predicted_token_logits, 
            ...
            prediction_result = pd.DataFrame({'id':test_file_ids,'predicted_probas':label_1_predicted_probas, 
                                              'label_0_predicted_probas':label_0_predicted_probas,'labels':labels})
            prediction_result.to_csv(os.path.join(save_path, f'result_logit_{file_name}.csv'), index=False)
            
            result_df = calc_metric(label_1_predicted_probas, labels)
            result_df.to_csv(os.path.join(save_path, f'result_thresholding_{file_name}.csv'), index=False)

            file_result = pd.DataFrame({'id':file_ids_list,'input_ids':input_ids_list, 'input_tokens':input_tokens,
                                        'spk_type_ids_list':spk_type_ids_list, 'predicted_token_logits':label_1_predicted_token_logits,
                                        'label_0_predicted_token_logits':label_0_predicted_token_logits, 'label':label_per_token_logits})
            file_result.to_csv(os.path.join(save_path, f'result_file_{file_name}.csv'), index=False)
            print()

def parse_args():
    parser = argparse.ArgumentParser(description='Training : GPT model : CTC_Loss')
    parser.add_argument('--SEED', type=int, default=17, help='Random Seed')
    parser.add_argument('--WORLD_SIZE', type=int, default=2, help='number of distributed processes')
    parser.add_argument('--PORT', type=str, default='12234', help='number of Master PORT Number')
    parser.add_argument("--IS_FULL", type=str2bool, default=False)
...
"""
CUDA_VISIBLE_DEVICES=1,2,3 python main.py --WORLD_SIZE 3 --IS_FULL False --IS_AVGPOOL False
CUDA_VISIBLE_DEVICES=1,2,3 python main.py --WORLD_SIZE 3 --IS_FULL False --IS_AVGPOOL True

CUDA_VISIBLE_DEVICES=1,2,3 python main.py --WORLD_SIZE 3 --IS_FULL True --IS_AVGPOOL False
CUDA_VISIBLE_DEVICES=1,2,3 python main.py --WORLD_SIZE 3 --IS_FULL True --IS_AVGPOOL True
